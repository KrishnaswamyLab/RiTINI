...
def infer_graphs_at_timepoints(
    ...
    prior_adjacency,
    device,
    threshold=None  # Changed default to None
):
    """
    Infer graphs at each timepoint using the trained model.
    Now prints edges that changed at each timepoint.
    ...
    Args:
        ...
        threshold: Attention weight threshold for edge inclusion. 
                   If None, uses median of all attention weights.
    ...
    # Convert NumPy array to tensor before using torch.nonzero
    prior_adjacency_tensor = torch.tensor(prior_adjacency, dtype=torch.float32)
    edge_index = torch.nonzero(prior_adjacency_tensor, as_tuple=False).t().to(device)

    n_genes = node_features.shape[1]
    
    # Track previous graph edges for comparison
    previous_edges = None
    
    # Collect all attention weights if threshold is None
    ...
    with torch.no_grad():
        for t in range(node_features.shape[0]):
            ...
            for i, (src, dst) in enumerate(edge_idx.t().cpu()):
                ...
                # Collect attention weights for median calculation
                if threshold is None:
                    all_attention_weights.append(weight)

            attention_matrices.append(attention_matrix.numpy())
        
        # Calculate median threshold if not provided
        if threshold is None:
            threshold = np.median(all_attention_weights)
            print(f"Using median threshold: {threshold:.4f}")
        
        # Now create graphs with the determined threshold
        for t in range(node_features.shape[0]):
            attention_matrix = attention_matrices[t]
            ...
            if previous_edges is not None:
                ...
                if not added_edges and not removed_edges:
                    print("No changes from previous timepoint")
            else:
                # First timepoint - just report initial edges
                print(f"\n=== Timepoint {t} (Initial) ===")
                print(f"Total edges: {len(current_edges)}")
                if current_edges:
                    print("Initial edges:")
            ...
            # Update previous edges for next iteration
...
def visualize_graphs_at_timepoints(
    ...
    for t in timepoints_to_show:
        ...
        im = ax2.imshow(attention_matrices[t], cmap='Blues', aspect='auto')
    ...
    print(f"\nSaved {len(timepoints_to_show)} graph visualizations to {save_dir}/")
    ...
    for idx, t in enumerate(selected_timepoints):
        ...
        if len(weights) > 0:
            nx.draw_networkx_edges(G, pos, width=[w*2 for w in weights],
                                  alpha=0.6, edge_color=weights,
                                  edge_cmap=plt.cm.Blues, ax=axes[idx])
