...
class sergio (object):
    ...
    splice_ratio = 4, dt_splice = 0.01, migration_rate = None):
        ...
        if np.isscalar(noise_params):
            ...
            logger.error("expect one noise parameter per gene")
            raise ValueError("Error: expect one noise parameter per gene")
        ...
        if np.isscalar(decays) == 1:
            ...
            logger.error("expect one decay parameter per gene")
            raise ValueError("Error: expect one decay parameter per gene")
            sys.exit()


        if self.dyn_:
            if (self.bifurcationMat_ == None).any():
                logger.error("Bifurcation Matrix is missing")
                raise ValueError("Error: Bifurcation Matrix is missing")
                sys.exit()
            ...
            if noise_params_splice == None:
                ...
                logger.error("expect one splicing noise parameter per gene")
                raise ValueError("Error: expect one splicing noise parameter per gene")
                sys.exit()
            ...
            if np.isscalar(splice_ratio):
                ...
                logger.error("expect one splicing ratio parameter per gene")
                raise ValueError("Error: expect one splicing ratio parameter per gene")
                sys.exit()
        ...
        with open(input_file_taregts,'r') as f:
            ...
            if (shared_coop_state <= 0):
                for row in reader:
                    ...
                    ##################### Raise Error ##########################
                    if nRegs == 0:
                        logger.error("a master regulator (#Regs = 0) appeared in input")
                        raise ValueError("Error: a master regulator (#Regs = 0) appeared in input")
                        sys.exit()
                        ############################################################
